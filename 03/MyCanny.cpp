...
int MyCanny::readGrayImage(string inputFileName)
{
	try
	{
...
功能：
	保存检测后的灰度图
****************************************/
int MyCanny::writeGrayImage(string outputFileName, CImg<short int> image)
{
...
*************************************************************************************/
void MyCanny::gaussianSmooth(CImg<short int>* image, float sigma, CImg<short int>* smootheDim)
{
...
功能：
	得到高斯核
*************************************************************/
void MyCanny::makeGaussianKernel(float sigma, float ** kernel, int * windowsize)
{
	int i, center;
	float x, fx, sum = 0.0;
...
函数功能：
	计算x y偏导后的图像
***********************************************************/
void MyCanny::derrivativeXY(CImg<short int> *smootheDim,
...
double MyCanny::angle_radians(double x, double y)
{
	...
	if (x >= 0) {
		if (y >= 0) return(ang);
		else return(2 * MyPI - ang);
	}
	else {
		if (y >= 0) return(MyPI - ang);
		else return(MyPI + ang);
	}
}
...
*******************************************************************************/
void MyCanny::radianDirection(CImg<short int>* delta_x, CImg<short int>* delta_y, 
...
*/
void MyCanny::magnitudeXY(CImg<short int> * delta_x, CImg<short int> * delta_y, 
...
***********************************************************************************/
void MyCanny::nonMaxSuppression(CImg<short int>* magnitude, CImg<short int>* delta_x,
...
**************************************************************************************/
void MyCanny::applyHysteresis(CImg<short int>* magnitude, CImg<short int>* nonMaxSupp, 
...
功能： 删除图像中较短长度小于20的边缘，边缘长度定义为像素点的个数
	   ...
	   达到剔除短边缘的目的。
******************************************************************/
void MyCanny::cutShortEdge(CImg<short int>* edge, CImg<short int>* resultEdgeImage)
{
...
CImg<short int> MyCanny::getCutShortEdgeImage() {
    return this->cutShortEdgeImage;
}
CImg<short int> MyCanny::getDeltaXImage() {
    return this->deltaXImage;
}
CImg<short int> MyCanny::getDeltaYImage() {
    return this->deltaYImage;
}
/*
对输入文件进行边缘检测，然后得到输出图像
*/
void MyCanny::canny(string inputFileName) {
	// cout << "正在进行边缘检测..." << endl;
	/*1. 读取图像，并转化为灰度图*/
