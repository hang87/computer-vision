...
{
	if (DEBUG) cout << "save edge points" << endl;
	Point p;
	...
	cout << "num of edge points: " << (*edgePointSet).size() << endl;
}
...
{
	if (DEBUG) cout << "Judging three points collinearity" << endl;
	if(DEBUG) cout << x1 << " " << x2 << " " << y1 << " " << y2 << " " << z1 << " " << z2 << endl;
	if ((x1 * y2 - x2 * y1) + (y1 * z2 - y2 * z1) + (z1 * x2 - z2 * x1) == 0) {
		if (DEBUG) cout << "collinearity" << endl;
		return true;
	}
	if (DEBUG) cout << "not collinearity" << endl;
	return false;
}
...
void HoughForCircle::calculate_a_b_r(Circle* beingVerifiedCircle, vector<Point> edgePointSet, vector<Circle> haveVerifiedCircleSet)
{
	...
	while (1) {
		if (DEBUG) cout << "Calculating the parameters of a circle" << endl;
		...
		if (a < 0 || a > width || b  < 0 || b > height) {
			if (DEBUG) {
				cout << "Eliminating the Circle Center from the Image  " << endl;
				cout << a << " " << b << " " << r << endl;
			}
			continue;
...
{
	if (DEBUG) cout << "Determine whether a randomly generated circle is similar to or intersected with a determined circle" << endl;
	return  false;
...
void HoughForCircle::drawCircleByEquation(vector<Circle> haveVerifiedCircleSet, CImg<short int>& houghImage)
{
	...
	cout << haveVerifiedCircleSet.size();
	for (int i = 0; i < haveVerifiedCircleSet.size(); i++) {
		int a = haveVerifiedCircleSet[i].a;
		int b = haveVerifiedCircleSet[i].b;
		int r = haveVerifiedCircleSet[i].r;
...
void HoughForCircle::hough(string fileName)
{
	...
	houghImage.display();
	cout << "num of coins: " << haveVerifiedCircleSet.size() << endl;
